...
//addresse de celui qui recoit les tokens
const receiverAddress = "0xb3726d011CB83Eaffb19712071de2dCF79B6710d"
...
async function main() {
    ...
    const accountEncodedData = Account.interface.encodeFunctionData("transferTo", [receiverAddress, AmountToReceive])
    ...
    const receiverBalance = await tk.balanceOf(receiverAddress)
    console.log("Les tokens ont été envoyés de l'adresse ", { sender }, "a l'adresse ", { receiverAddress })
    console.log("la balance du sender est ", Number(senderBalance), "Tk")
    console.log("la balance du receiver est ", Number(receiverBalance), "Tk")
